  ...
  function scanSharedFiles() {
    ...
    try {
      ...
      while (hasMore && totalCount < MAX_FILES) {
        ...
        var result = TangentCore.driveIterator(query, continuationToken, batchSize);
      ...
      // Log completion
      TangentCore.logActivity(user, 'cleaner', 'scanSharedFiles_completed: ' + totalCount + ' files');
      ...
      TangentCore.logActivity(user, 'cleaner', 'scanSharedFiles_error: ' + error.toString());
      return {
        success: false,
